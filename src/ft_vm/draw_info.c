...
static void	draw_main_info(t_vm *vm, t_win win, t_status *s)
{
	...
	mvwprintw(win.info, 7, 3, "* Total cycle : %d", (*vm).cycles);
...
void		draw_info(t_vm *vm)
{
	...
	c_tmp = ((double)(*vm).status.c_now / (double)(*vm).cycles_to_die * 20);
	ctd_tmp = ((double)(*vm).cycles_to_die / (double)CYCLE_TO_DIE * 20);
	draw_main_info(vm, (*vm).win, &(*vm).status);
	draw_counter((*vm).win, &c_tmp);
	draw_max_counter((*vm).win, &ctd_tmp);
	draw_animation((*vm).win.info, (*vm).cycles);
}
