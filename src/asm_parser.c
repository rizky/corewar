...
/*   Updated: 2018/04/15 23:20:22 by rnugroho         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	asm_check_op(t_asm *a)
{
	...
	i = 0;
	if (ARRAY_DATA(a->ops, a->op_c).opcode == -1)
		return (ft_error(OP, -1, a->file[a->i]));
	while (i < ARRAY_DATA(a->ops, a->op_c).param_c)
	{
		if (ARRAY_DATA(a->ops, a->op_c).params[i].type == -1)
